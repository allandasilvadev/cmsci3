...
class Home extends Frontend_Controller 
{
	public function __construct()
	{
		...
		$this->data['phones'] = $this->configuracoes->getMany( array( 'guia' => 'phone', 'status' => 'published' ) );
		$this->data['paginas'] = $this->paginas->getMany( array( 'status' => 'published' ) );

		$published_posts = $this->posts->getMany( array( 'status' => 'published', 'published_at' => NULL, 'visibility' => 'public' ) );
		$published_at_posts = $this->posts->getMany( array( 'status' => 'published', 'published_at <' => date( 'Y-m-d H:i:s' ), 'visibility' => 'public' ) );
		$this->data['posts'] = array_merge( $published_posts, $published_at_posts );

		$this->infos();
	}

	protected function paginas_visitadas( $hash = '' ) {
		/* Views */
	...
	protected function posts_visitados( $hash = '' ) {
		/* Views */
				$views = $this->views->get_by( array( 
					'post_type' => 'post', 
					'post_type_hash' => $hash 
	...
	public function index()
	{
		...
		$this->data['description'] = 'Bem-vindo ao meu blog';
	...
	public function paginas( $slug = '' )
	{
		if ( ! empty( $slug ) )
		{
			...
			$this->data['description'] = 'Páginas do blog.';

			$slug = strip_tags( addslashes( trim( $slug ) ) );
			$page = $this->paginas->getMany( array( 'slug' => $slug, 'status' => 'published' ) );

			if ( $page != null ) {
				$this->paginas_visitadas( $page[0]['hash'] );
				$this->data['page'] = $page;
				$this->render( 'home/paginas' );
			} else {
				redirect( base_url( '/' ) );
			}
		}
	...
	public function posts( $slug = '' )
	{
		if ( ! empty( $slug ) )
		{
			...
			$this->data['description'] = 'Posts do blog.';

			$slug = strip_tags( addslashes( trim( $slug ) ) );

			/* published posts */
			$post = $this->posts->get_by( array( 
				'slug' => $slug, 
				'status' => 'published', 
				'published_at' => NULL,
				'visibility' => 'public'
			...
			if ( $post != null ) {
				$this->posts_visitados( $post['hash'] );
				$this->data['post'] = $post;
